...
> Estado de listados: la distribución verified-live de hoy es Official MCP Registry (metadatos del paquete activos vía `server.json`) y Smithery; el listado de Glama también está activo (live) en [`https://glama.ai/mcp/servers/vk0dev-code-impact-mcp`](https://glama.ai/mcp/servers/vk0dev-code-impact-mcp), aunque su ruta pública badge-ready aún no está resuelta, por lo que el PR [`punkpeye/awesome-mcp-servers#5907`](https://github.com/punkpeye/awesome-mcp-servers/pull/5907) ya existe, no debe duplicarse y todavía no puede finalizarse con una badge URL estable. La entrada de Cline en el marketplace ([`cline/mcp-marketplace#1486`](https://github.com/cline/mcp-marketplace/issues/1486)) está abierta y aún no ha sido aceptada; MCP Hive sigue siendo un operator step manual de tipo submit-next en lugar de un listado activo (live) ya reclamado; y este README no afirma un estado badge-ready ni un awesome-listing finalizado.
...
Si estás eligiendo una herramienta para un agente o reviewer, la pregunta clave sigue siendo simple: ¿necesitas **explorar un grafo o un contexto de código más amplio**, o necesitas **gatear un cambio propuesto antes del commit**?

| Alternativa | Mejor para | Dónde gana hoy | Dónde gana CodeImpact MCP |
| --- | --- | --- | --- |
| **CodeImpact MCP** | Dependency gating decision-first para cambios TS/JS propuestos, incluso en monorepos | Salida inmediata PASS/WARN/BLOCK, `detect_cycles` integrado, workspace-aware gate checks, file-level blast-radius triage, soporte Python acotado para `analyze_impact` y `gate_check`, workflow local-first y un helper directo de Husky `install-hook` | Es la mejor opción cuando el trabajo es “¿esto es seguro de commitear?” y no “ayúdame a explorar todo el repo” |
| **code-graph-mcp** | Inspección de code graphs hosted o prebuilt a través de una MCP surface | Gana cuando el agente necesita graph traversal, semantic graph queries y acceso a grafos public/private a través del flujo ya existente de DeepGraph o CodeGPT, en lugar de un CLI local centrado primero en el gate | Gana cuando quieres un único verdict pre-commit acotado, con triage de archivos afectados, en lugar de una sesión de graph exploration |
| **Depwire** | Inteligencia de dependencias y workflows de arquitectura sobre una surface más amplia de lenguajes y tooling | Gana cuando necesitas symbol-level analysis, visualización en navegador, workflows de security/health o una plataforma multi-language más amplia que la que CodeImpact apunta intencionalmente | Gana cuando quieres una herramienta pequeña con licencia MIT, que siga siendo local-first, ya esté live en el Official MCP Registry y responda rápido a una gating question estrecha |
| **RepoGraph** | Repository-level graph retrieval para SWE-style context gathering | Gana cuando el workflow es más research-heavy o retrieval-heavy, especialmente si necesitas line-level repo context para ciclos mayores de entendimiento del repo y no un check ligero al momento del commit | Gana cuando los touched files ya son conocidos y solo necesitas bounded blast-radius triage más un gate result |
| **CodeGraphContext** | Graph database local consultable más un CLI/MCP de code understanding más amplio | Gana cuando el agente necesita una queryable local graph database, contexto multi-language más amplio y un razonamiento de repositorio más largo que un gate decision-first | Gana cuando quieres salida decision-first desde un gate local y no un workflow más amplio basado en graph database |
| **Manual follow-up tipo MCP Hive** | Envío manual a marketplace/discovery una vez que el repo truth ya está estable | Gana cuando el trabajo es marketplace packaging, screenshots y operator copy para un directory workflow, no el dependency gate en sí | Gana cuando primero necesitas el wedge ya shipped del producto: local verdicts, install-hook wiring y Python impact checks acotados, antes de cualquier listing follow-up manual |
...
**Elige una de las alternativas cuando:** el trabajo principal es hosted/public graph access, graph exploration, entender el repo, cubrir workflows de dependencias más amplios, recuperar contexto apoyado en graph database para reasoning loops más largos o hacer manual marketplace packaging después de fijar el core repo surface.

...
> Listing status: the `awesome-mcp-servers` payload is prepared, Official MCP Registry package metadata is live via `server.json`, Glama still needs a real manual listing plus badge before the external PR lane can move, and MCP Hive is a manual submit-next operator step, not a currently claimed live listing. This README does not claim any new third-party listing as live unless it is already verified.

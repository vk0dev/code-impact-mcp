...
| **CodeGraphContext** | Broader local code graph and context platform with dual CLI + MCP entrypoints | Better when the agent needs queryable local graph/indexing breadth, wider multi-language context workflows, and longer-form repository reasoning instead of a decision-first gate | Better when you want a fast local PASS/WARN/BLOCK gate for known file changes, not a broader graph/context platform |

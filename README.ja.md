...
`.husky/` がすでに存在する場合、このコマンドは `.husky/pre-commit` 内の `code-impact-mcp` と印の付いたブロックだけを作成または更新し、他の hook 内容はそのまま残します。Husky がまだ初期化されていない場合は、hook 基盤を勝手に作らず、実行可能なメッセージを返して停止します。
